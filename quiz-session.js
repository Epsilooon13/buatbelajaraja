(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.QuizSession=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function pack(state,fingerprint,now=Date.now()){
    return {version:2,status:'active',id:state.id,mode:state.mode,seconds:state.seconds,index:state.index,deadline:state.deadline,savedAt:now,config:{...state.config,language:state.config.language||'id'},
      questions:state.session.map(q=>({id:q.id,fingerprint:fingerprint(q),category:q.category,language:q.language||'id',order:q.options.map(o=>o.key),selected:q.selected,timedOut:!!q.timedOut}))};
  }
  function restore(data,bank,fingerprint){
    const invalid=()=>({status:'invalid'});
    if(!data||data.status==='completed')return {status:'empty'};
    if(![1,2].includes(data.version)||data.status!=='active'||typeof data.id!=='string'||!data.id||data.id.length>150)return invalid();
    ...
    const language=data.version===1?'id':config.language;
    if(!['id','en'].includes(language))return invalid();
    ...
    for(const [i,row] of data.questions.entries()){
      ...
      if(data.version===2&&(!['id','en'].includes(row.language)||row.language!==language))return invalid();
      if((q.language||'id')!==language)return {status:'changed'};
    ...
    return {status:'ready',state:{id:data.id,mode:data.mode,seconds:data.seconds,index:data.index,deadline:data.deadline,config:{...config,language,materials:config.materials.map(m=>({...m}))},session}};
  }
